...
router.get(
  ...
  asyncHandler<AuthRequest>(async (req, res) => {
    ...
    const livestock = await prisma.livestock.findMany({
      ...
      include: { healthEvents: { orderBy: { createdAt: "desc" }, take: 2 } },
      orderBy: { species: "asc" }
    });
    res.json({ livestock });
  })
...
router.post(
  ...
  asyncHandler<AuthRequest>(async (req, res) => {
    ...
    const livestock = await prisma.livestock.create({ data: { ...input, farmId: req.params.farmId } });
...
router.put(
  ...
  asyncHandler<AuthRequest>(async (req, res) => {
    await getOwnedFarm(req.user.id, req.params.farmId);
    const existing = await prisma.livestock.findFirst({ where: { id: req.params.id, farmId: req.params.farmId } });
    if (!existing) throw new AppError(404, "Livestock group not found");
    const input = livestockSchema.partial().parse(req.body);
    const livestock = await prisma.livestock.update({ where: { id: req.params.id }, data: input });
    res.json({ livestock });
  })
);
